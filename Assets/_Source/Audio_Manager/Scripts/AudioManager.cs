    ...
    public class AudioManager:MonoBehaviour {
        ...
        private void Update() {
            for (int i = _playing.Count - 1;i >= 0;i--) {
                ...
                if (h.autoRelease && !h.source.isPlaying) {
                    _playing.RemoveAt(i);
                    try { h.onComplete?.Invoke(); } catch (Exception e) { Debug.LogException(e); }
        ...
        public void StopSfx(AudioSource handle) {
            ...
            for (int i = 0;i < _playing.Count;i++) {
                if (_playing[i].source != handle)
                    continue;
                var pool = _playing[i].pool;
                _playing.RemoveAt(i);
                pool.Release(handle);
                return;
            }
        ...
        public void SetVolume(AudioChannel channel,float linear01) {
            ...
            if (v > MuteEpsilon) { _volume[channel] = v; _muted[channel] = false; } else { _muted[channel] = true; }
